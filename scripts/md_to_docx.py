"""
Convert Interim_Report_Arka_Talukder.md to Word with nice formatting.
- Title page, proper headings, lists, tables, equations, Figure 1 PNG
- Run: python scripts/md_to_docx.py
- Output: B01821011_Interim_Report_Final.docx
"""
...
def add_run_with_format(p, text):
    """Add text handling **bold** and *italic*."""
    remaining = text
    while remaining:
        b = remaining.find("**")
...
def parse_table(lines: list, i: int) -> tuple[list, list, int]:
    parts = lines[i].split("|")
    headers = [p.strip() for p in parts[1:-1] if parts[1:-1]]
    if not headers:
...
def latex_to_display(latex: str) -> str:
    ...
    s = re.sub(r'F_1', 'F\u2081', s)  # subscript 1
    s = re.sub(r'\s+', ' ', s)
    return s.strip()
...
def parse_md_to_docx(md_path: Path, out_path: Path) -> None:
    ...
    # Find start of main content (skip header until ## Introduction)
    start_i = 0
    ...
    while i < len(lines):
        ...
        if img_match:
            ...
            for base in [md_path.parent / path, md_path.parent / "figure1_pipeline.png", PROJECT_ROOT / "figure1_pipeline.png"]:
                img_path = Path(base).resolve()
                if img_path.exists():
                    p = doc.add_paragraph()
                    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        if stripped.startswith("|") and "|" in stripped[1:]:
            ...
            if headers:
                add_formatted_table(doc, headers, rows, pending_table_caption)
                pending_table_caption = None
            continue
