"""
Sensitivity analysis and multi-seed central GNN experiments.
...
Outputs:
  ...
  results/figures/sensitivity.png
"""
import argparse
import copy
import csv
...
from src.models.trainer import compute_class_weights, evaluate, measure_inference_time
...
def run_sensitivity(cfg):
    ...
    for ws in window_sizes:
        for k in knn_ks:
            ...
            results.append(row)
            logger.info(
                "  Result: F1=%.4f AUC=%.4f inf=%.1fms (%.0fs)",
                row["f1"],
                row["roc_auc"],
                row["inference_ms"],
                elapsed,
...
def plot_sensitivity(results):
    ...
    out_png = FIG_DIR / "sensitivity.png"
    fig.savefig(out_png, dpi=300, bbox_inches="tight")
    plt.close(fig)
    logger.info("Sensitivity plot saved -> %s", out_png)
