...
| **Submission procedure / checklist (Markdown)** | [`docs/reports/SUBMISSION_CHECKLIST.md`](../docs/reports/SUBMISSION_CHECKLIST.md) |
| **Handbook alignment notes (if used)** | [`docs/reports/HANDBOOK_COMPLIANCE_REPORT.md`](../docs/reports/HANDBOOK_COMPLIANCE_REPORT.md) |
...
| **Meeting / feedback evidence map** | [`docs/reports/SUPERVISOR_FINAL_FEEDBACK.md`](../docs/reports/SUPERVISOR_FINAL_FEEDBACK.md) |
| **Narrative + Q&amp;A (long-form)** | [`../DISSERTATION_PROJECT_GUIDE.md`](../DISSERTATION_PROJECT_GUIDE.md) |
| **Viva brief, demo script, recording notes** | [`docs/viva/`](../docs/viva/) |

**Suggested order in a final meeting:** final Word (link above) → `docs/reports/SUPERVISOR_FINAL_FEEDBACK.md` → `config/experiment.yaml` → `results/metrics/` + one figure from `results/figures/` → `src/siem/api.py` if runtime is discussed.
...
results/                    # Local outputs: metrics/, figures/, checkpoints/, alerts/ (as committed or generated)
Arka_Talukder_Dissertation_Final_DRAFT.md  # Dissertation text source (images: assets/, results/figures/)
submission/                 # Final .docx, PDF (optional), school forms
project_portfolio/          # This “start here” guide for GitHub and stakeholders
supervisor_package/         # Zipped / shared layout for supervisor (mirror + checklist)
viva_supervisor_materials/  # Screen-share etiquette and walkthrough
docs/                       # Reports, checklists, viva, reference templates
archive/                    # Interim work, process/attendance (appendix sources), one-off scripts
```
