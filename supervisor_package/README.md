...
**Sensitivity / multi-seed:** In the **current repo**, the driver is **`scripts/run_sensitivity_and_seeds.py`** (project root). Extra packaged figure scripts may live under **`artifacts/main_report_figures/`** — prefer the root `scripts/` version when demonstrating the live pipeline.
