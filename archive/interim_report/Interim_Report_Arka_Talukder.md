...
In this section I critically summarise the current academic literature in my subject area and provide the academic framework for my research. I evaluate the strengths and limitations of prior work and identify the gap that this project addresses.
...
Deep learning approaches such as MLPs can learn complex patterns without manual feature engineering (Shone et al., 2018). Shone et al. (2018) used deep autoencoders with classifiers on NSL-KDD with good results. A limitation is that NSL-KDD is smaller and less diverse than CICIoT2023, and they did not consider temporal or relational structure between flows. Both Random Forest and MLP treat each flow independently and ignore links between flows—a gap that graph-based methods can address.
...
Basak et al. (2025) used GNNs for intrusion detection on UNSW-NB15. They built graphs from flow features with edges linking similar flows, achieving F1 over 97%. However, a key limitation is that they used static graphs—the whole dataset was one snapshot—so they did not capture how behaviour changes over time. UNSW-NB15 is also older and has fewer attack types than CICIoT2023, limiting generalisability to current IoT threats.

Ngo et al. (2025) used GNNs on IoT datasets with kNN-based graph construction. Edges linked flows similar in feature space, which worked without device topology. They validated k=5 as a good default. A critical gap remains: they used static graphs with no temporal modelling. Their system cannot capture how attacks evolve over time, which is important for slow attacks or multi-phase campaigns.
...
Lazzarini et al. (2023) used federated learning for intrusion detection on UNSW-NB15. Their federated MLP achieved within 2% F1 of the centralised model. A significant limitation is that they used only two clients with IID data—an ideal case that does not reflect real IoT deployments where different networks (e.g. factory sensors vs. smart homes) see different traffic. Non-IID data is known to hurt FedAvg performance (Kairouz et al., 2021).

Albanbay et al. (2025) addressed non-IID data using Dirichlet splits. Lower alpha (more heterogeneous data) caused up to 8% F1 drop and required about twice as many rounds to converge. However, they used flat models, not graphs. It remains unclear whether graph-based models behave differently under non-IID conditions. My project tests this by combining GNNs with federated learning using alpha=0.5.
...
Alabbadi and Bajaber (2025) used SHAP for IoT intrusion detection. Feature attribution helped analysts trust the alerts. A practical limitation is that SHAP is slow for deep models—one prediction can take over 10 seconds, which is too slow for real-time or edge use. Integrated Gradients is faster, requiring only one forward and backward pass, and thus better suited to edge deployment.

GAT attention weights provide an additional form of explanation by showing which neighbour flows influenced each classification. No prior work combines both: feature attributions from Integrated Gradients and structural explanations from attention. I use both in this project to give analysts richer context. Alabbadi and Bajaber (2025) did not explore this dual approach.
...
In this section I describe in detail how I am conducting my research. I make clear the basis of my research method (a quantitative, experimental development project), how I gather, analyse and interpret results, and the verifiable academic goal. I also explain how I will present the data to demonstrate the rigor of the process and my interpretation of the results.
...
My project has a clear verifiable academic goal: to test whether an explainable dynamic GNN with federated learning can detect IoT attacks and produce useful SIEM alerts. I will demonstrate rigor through:

1. **Architectural novelty:** As far as I know, no other prototype combines dynamic GNN (GAT+GRU), federated learning, dual explainability and SIEM integration in one system on CICIoT2023 (Table 1, Section 1.6).
2. **Empirical evidence:** I compare flat models, centralised GNN and federated GNN on the same data with identical preprocessing. This provides objective evidence of whether graphs and federated learning add value.
3. **Practical applicability:** The full pipeline runs on CPU with acceptable latency for edge use, bridging research and real SOC tools.

**Data presentation for rigor:** I will present results in tables (precision, recall, F1, ROC-AUC, inference time), confusion matrices, ROC curves, and FL convergence plots. Each hypothesis (H1–H3) maps to specific metrics that can be verified on the held-out test set. I will not use the test set for any tuning; it is reserved solely for final evaluation.

### 2.9 Anticipated Critical Reflection

The handbook requires a Critical Reflection section in the final report. I anticipate including the following areas, which I will expand upon once results are available:
...
These limitations define clear boundaries for the conclusions that can be drawn and identify specific directions for future work. I will reflect on how well the project was conducted, any errors made, and the reliability and significance of the results in the Critical Reflection section of my final report.

---

## 3. Plan for Completion

This section describes the current status of my project and how I intend to progress it to completion. I describe the technical work remaining, how results will be analysed and evaluated, and—as the handbook recommends—how I will proceed if the results are in some way deficient (e.g. if the GNN does not outperform baselines or federated learning degrades performance).
